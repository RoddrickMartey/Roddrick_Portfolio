  ...
  return (
    ...
    >
      ...
      <div className="flex flex-col flex-1 gap-3 p-5">
        <h3 className="text-base font-semibold text-foreground leading-snug">{title || "Untitled Project"}</h3>
        <p className="text-sm font-light leading-relaxed text-muted-foreground flex-1 line-clamp-3">{summary || "No summary provided."}</p>
...
function ProjectPage() {
  ...
  if (isError) {
    const msg = error?.response?.data?.message || error?.message || "Failed to load projects.";
    return (
      <div className="flex flex-col items-center justify-center min-h-screen gap-4 p-6">
        <Error message={msg} />
        <Button type="button" size="sm" variant="outline" onClick={() => refetch?.()} className="flex items-center gap-2 rounded-full">
  ...
  return (
    <main className="relative min-h-screen text-foreground">
      <section className="max-w-6xl mx-auto px-6 pt-28 pb-24 md:px-12 lg:px-20 flex flex-col gap-12">
        ...
        >
          <p className="text-xs font-mono tracking-[4px] text-primary/50 uppercase">My Work</p>
          <div className="flex items-center gap-4">
            <h1 className="text-4xl sm:text-5xl font-bold tracking-tight leading-tight flex items-center gap-3">
              <Laptop className="w-8 h-8 text-primary" />
              Projects
            </h1>
            <span className="px-3 py-1 rounded-full border border-border bg-muted/50 text-sm font-mono text-muted-foreground">{total}</span>
          </div>
