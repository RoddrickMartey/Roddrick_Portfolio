    ...
    <main className="relative min-h-screen overflow-hidden text-foreground">
      <div className="max-w-6xl mx-auto px-6 pt-28 pb-28 md:px-12 lg:px-20 flex flex-col gap-24">
        ...
        <div className="flex flex-col items-center gap-12 md:flex-row md:items-center md:gap-20">
          <motion.div {...fadeUp(0)} className="flex-shrink-0 w-[200px] sm:w-[240px] md:w-[280px] lg:w-[320px]">
            <ImageShowcase src={data.aboutImage} caption="That's me" />
          </motion.div>
          <div className="flex flex-col gap-6 text-center md:text-left">
            <motion.div {...fadeUp(0.1)}>
              <p className="text-xs font-mono tracking-[4px] text-primary/60 uppercase mb-2">Full-Stack Developer</p>
              <h1 className="text-4xl sm:text-5xl lg:text-6xl font-bold tracking-tight leading-[1.1]">{data.fullName}</h1>
            </motion.div>
            <motion.p {...fadeUp(0.2)} className="max-w-xl text-base md:text-lg font-light leading-relaxed text-muted-foreground">
              {data.bio[0]}
            </motion.p>
        ...
        <motion.div {...fadeUp(0.2)} className="flex flex-col gap-6 max-w-3xl mx-auto w-full">
          ...
          {data.bio.slice(1).map((para, i) => (
            <motion.p key={i} {...fadeUp(0.25 + i * 0.05)} className="text-base md:text-lg font-light leading-relaxed tracking-wide text-muted-foreground">
              {para}
            </motion.p>
          ))}
        </motion.div>
          ...
          <div className="flex flex-wrap gap-3">
            {data.techStack.map((tech, i) => (
              ...
              >
                ...
                <p className="relative text-sm font-medium text-foreground/80 group-hover:text-foreground transition-colors">{tech}</p>
              </motion.div>
            ))}
          </div>
        </motion.div>
